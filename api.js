...
const API = (() => {
    ...
    function parseChain(rawChain, spot) {
        ...
        for (const item of rawChain) {
            ...
            if (item.call_options) {
                ...
                strikes[strike].CE = {
                    ...
                    oi: c.oi || 0,
                    prev_oi: c.prev_oi ?? null,
                    iv: g.iv != null ? (g.iv < 1 ? g.iv * 100 : g.iv) : (c.iv ?? null),
                    delta: g.delta ?? null,
                    theta: g.theta ?? null,
                    gamma: g.gamma ?? null,
                    vega: g.vega ?? null,
            ...
            if (item.put_options) {
                ...
                const gp = item.put_options.option_greeks || {};
                strikes[strike].PE = {
                    ltp: p.ltp || p.last_price || 0,
                    bid: p.bid_price || p.ltp || 0,
                    ask: p.ask_price || p.ltp || 0,
                    oi: p.oi || 0,
                    prev_oi: p.prev_oi ?? null,
                    iv: gp.iv != null ? (gp.iv < 1 ? gp.iv * 100 : gp.iv) : (p.iv ?? null),
                    delta: gp.delta ?? null,
                    theta: gp.theta ?? null,
                    gamma: gp.gamma ?? null,
                    vega: gp.vega ?? null,
        ...
        if (atmData?.CE?.iv && atmData?.PE?.iv) {
            ...
            debugLog('ATM_IV_CALC', { method: 'chain_greeks', ceIv: (atmData.CE.iv / 100).toFixed(4), peIv: (atmData.PE.iv / 100).toFixed(4), atmIv: atmIv.toFixed(2) });
        }
