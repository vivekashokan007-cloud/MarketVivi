...
function updateWatchStatusHint(serviceStatus = null) {
    ...
    const expectedFullDay = Number.isFinite(serviceStatus?.expectedPollsFullDay) ? serviceStatus.expectedPollsFullDay : 78;
...
function sanitizeTradeForInsert(trade = {}) {
    ...
    // R2.6: Paper provenance lives in entry_snapshot.paper_test (+ compact mirrors).
    // Until schema capability is proven, strip unsupported top-level trades_v2 fields
    ...
    clone.entry_snapshot = {
        ...
        ...(paperTest ? {
            ...
            selection_source: paperTest.selection_source || entrySnapshot.selection_source || null,
            paper_policy_version: paperTest.paper_policy_version || entrySnapshot.paper_policy_version || null,
            brain_authorization_id: paperTest.brain_authorization_id || entrySnapshot.brain_authorization_id || null,
        } : {}),
    };
    ...
    delete clone.paper_lane;
    delete clone.selection_source;
...
function filterPaperRowsForSameSessionTeacherComparison(rows = []) {
    const kept = [];
    const excluded = { OVERNIGHT: 0, MULTIDAY: 0, OPEN: 0, UNKNOWN: 0, OTHER: 0 };
    for (const row of Array.isArray(rows) ? rows : []) {
        if (!row || typeof row !== 'object') { excluded.OTHER += 1; continue; }
        const meta = deriveHoldingHorizon(row);
        const horizon = meta.holding_horizon || 'UNKNOWN';
        if (meta.comparable_for_teacher_pool === true) {
            kept.push(Object.assign({}, row, { holding_horizon: horizon, comparable_for_teacher_pool: true }));
        } else if (Object.prototype.hasOwnProperty.call(excluded, horizon)) {
            excluded[horizon] += 1;
        } else {
            excluded.OTHER += 1;
        }
    ...
    return {
        ...
        note: 'No active Paper-vs-teacher pooling path currently consumes this filter in UI ranking; use when comparison is built.'
    };
}
...
function normalizePaperIndexKey(raw) {
    const text = String(raw || '').trim().toUpperCase();
    if (text === 'NF' || text === 'NIFTY' || text === 'NIFTY 50') return 'NF';
    if (text === 'BNF' || text === 'BANKNIFTY' || text === 'NIFTY BANK') return 'BNF';
    return null; // never invent NF/BNF
}

function parsePositiveIntegralLotJs(raw) {
    if (raw === null || raw === undefined || raw === '') {
        return { status: 'field_absent', value: null, error: null, raw: null };
    }
    const n = typeof raw === 'number' ? raw : Number(String(raw).trim());
    if (!Number.isFinite(n)) {
        return { status: 'field_present_but_invalid', value: null, error: 'malformed_lot', raw };
    }
    if (n <= 0) {
        return { status: 'field_present_but_invalid', value: null, error: 'non_positive_lot', raw };
    }
    if (Math.abs(n - Math.round(n)) > 1e-9) {
        return { status: 'field_present_but_invalid', value: null, error: 'fractional_lot', raw };
    }
    const iv = Math.round(n);
    if (iv <= 0) {
        return { status: 'field_present_but_invalid', value: null, error: 'non_positive_lot', raw };
    }
    return { status: 'field_present', value: iv, error: null, raw };
}

function paperContractIdentityGate(candidate) {
    const reasons = [];
    const indexKey = normalizePaperIndexKey(candidate?.index || candidate?.index_key);
    if (!indexKey) reasons.push('index must be exactly NF or BNF');

    const ci = (candidate?.contract_identity && typeof candidate.contract_identity === 'object')
        ? candidate.contract_identity
        : null;
    const lotConflict = candidate?.lot_conflict === true
        || ci?.lot_conflict === true
        || candidate?.contract_identity_quarantine === true
        || ci?.contract_identity_quarantine === true;
    if (lotConflict) reasons.push('contract identity conflict or quarantine');

    const identityComplete = candidate?.identity_complete === true
        || ci?.identity_complete === true;

    // R2.5: require positive INTEGRAL contract lot; reject fractional (e.g. 65.5).
    const lotObs = parsePositiveIntegralLotJs(
        candidate?.contract_lot_size
        ?? ci?.contract_lot_size
        ?? candidate?.lotSize
        ?? candidate?.lot_size
        ?? ci?.lot_size
    );
    if (lotObs.status === 'field_absent') reasons.push('positive integral contract lot is required');
    if (lotObs.status === 'field_present_but_invalid') {
        reasons.push(`contract lot ${lotObs.error || 'invalid'}`);
    }
    const lotSize = lotObs.status === 'field_present' ? lotObs.value : null;

    const nLotsObs = parsePositiveIntegralLotJs(
        candidate?.number_of_lots ?? ci?.number_of_lots ?? candidate?.lots ?? 1
    );
    // missing number_of_lots defaults to 1 (one-lot path)
    let numberOfLots = 1;
    if (candidate?.number_of_lots != null || ci?.number_of_lots != null || candidate?.lots != null) {
        if (nLotsObs.status !== 'field_present') {
            reasons.push(`number_of_lots ${nLotsObs.error || 'invalid'}`);
            numberOfLots = null;
        } else {
            numberOfLots = nLotsObs.value;
        }
    }
    let quantityUnits = null;
    const qtyObs = parsePositiveIntegralLotJs(candidate?.quantity_units ?? ci?.quantity_units);
    if (qtyObs.status === 'field_present_but_invalid') {
        reasons.push(`quantity_units ${qtyObs.error || 'invalid'}`);
    } else if (qtyObs.status === 'field_present') {
        quantityUnits = qtyObs.value;
    } else if (lotSize != null && numberOfLots != null) {
        quantityUnits = lotSize * numberOfLots;
    }
    if (lotSize != null && numberOfLots != null && quantityUnits != null) {
        if (quantityUnits !== lotSize * numberOfLots) {
    ...
    return {
        ...
        lotFieldStatus: lotObs.status,
        reasons,
        ok: reasons.length === 0 && indexKey != null && lotSize != null && numberOfLots != null && !lotConflict,
    };
}
...
function paperTradeAuthorization(candidate) {
    ...
    return {
        ...
        number_of_lots: identity.numberOfLots,
        quantity_units: identity.quantityUnits,
...
function paperAnalysisAuthorization(candidate) {
    ...
    return {
        ...
        number_of_lots: identity.numberOfLots,
        quantity_units: identity.quantityUnits,
        legacy_boolean_diagnostic: legacyBoolean,
    };
}

function paperAnalysisAlternatives(indexKey, watchlistIds, atm) {
    const generated = Array.isArray(bd?.generated_candidates) ? bd.generated_candidates : [];
    const seen = new Set((watchlistIds || []).map(id => String(id)));
    const rows = [];
    for (const cand of generated) {
        if (!cand || String(cand.index || '').toUpperCase() !== String(indexKey || '').toUpperCase()) continue;
        if (seen.has(String(cand.id))) continue;
        const auth = paperAnalysisAuthorization(cand);
        if (!auth.allowed) continue;
        rows.push(cand);
        seen.add(String(cand.id));
        if (rows.length >= PAPER_ANALYSIS_ALTERNATIVES_CAP) break;
    }
    return rows;
}
...
function confirmPaperTest(candidate, finalAuthorization, capacity, opts = {}) {
    ...
    const lane = opts.paper_lane === 'paper_analysis' ? 'paper_analysis' : 'paper_primary';
    if (lane === 'paper_analysis') {
        return confirm(
            `📋 PAPER ANALYSIS — EXPERIMENTAL LANE — ${candidate.index} ${friendlyType(candidate.type)}\n\n`
            + 'This is NOT the primary Paper recommendation.\n'
            + 'No broker or sandbox order will be sent. Records an experimental paper_analysis observation only.\n'
            + 'Real/Sandbox remain blocked for this alternative.\n\n'
            + `Current brain status:\n${blockText}\n\n`
            + `Paper capacity: ${capacity.count}/${capacity.limit} active for ${candidate.index}.\n\n`
            + 'Continue with PAPER ANALYSIS (experimental)?'
        );
    }
    return confirm(
        `📋 PAPER TEST — ${candidate.index} ${friendlyType(candidate.type)}\n\n`
        + 'No broker or sandbox order will be sent. This records a managed paper_primary observation.\n\n'
        + `Current brain status:\n${blockText}\n\n`
        + `Paper capacity: ${capacity.count}/${capacity.limit} active for ${candidate.index}.\n\n`
...
async function takeTradeImpl(candidateId, isPaper = false) {
    ...
    if (isPaper && !confirmPaperTest(cand, finalAuthorization, paperCapacity, { paper_lane: paperLane })) {
        return;
    }
    // Real trade: confirm
    ...
    const paperIdx = normalizePaperIndexKey(cand.index);
    // R2.5: after authorization, use EXACTLY authorized lotSize / number_of_lots / quantity_units.
    // Never fall back to C.NF_LOT / C.BNF_LOT after contract authorization.
    ...
    if (isPaper && paperAuthorization && !paperObservationOnly) {
        ...
        if (!(Number.isFinite(entryLotSize) && entryLotSize > 0 && Number.isInteger(entryLotSize))) {
            alert('Paper trade blocked: authorized integral lotSize required (no constant fallback after auth).');
            return;
        }
        if (!(Number.isFinite(entryNumberOfLots) && entryNumberOfLots > 0 && Number.isInteger(entryNumberOfLots))) {
            alert('Paper trade blocked: authorized integral number_of_lots required.');
            return;
        }
        if (!(Number.isFinite(entryQuantityUnits) && entryQuantityUnits > 0)) {
            entryQuantityUnits = entryLotSize * entryNumberOfLots;
        }
        if (entryQuantityUnits !== entryLotSize * entryNumberOfLots) {
            alert('Paper trade blocked: authorized quantity_units disagree with lotSize × number_of_lots.');
            return;
        }
        // Block if authorized values disagree with nested identity when both present.
        const nestedLot = cand?.contract_identity?.contract_lot_size ?? cand?.contract_lot_size;
        if (nestedLot != null && Number(nestedLot) !== entryLotSize) {
        ...
        entryQuantityUnits = entryLotSize;
    }
    const executionMode = isPaper
        ? 'paper'
        : (String(cand.executionReadiness?.mode || 'paper').toLowerCase());
    const entryTimestamp = new Date().toISOString();
    const paperPolicyVersion = bd.brain_version || bd.brainVersion || bd.paper_policy_version || 'paper_policy_v1_20260913';
    const brainAuthId = paperLane === 'paper_analysis'
        ? (paperAnalysisAuth?.authorization_id
            || cand?.paperAnalysisEligibility?.authorization_id
            || cand?.paperAnalysisEligibility?.result_id
            || cand?.paperAnalysisEligibility?.id
            || null)
        : (finalAuthorization?.authorization_id || finalAuthorization?.result_id || null);
    const paperTestProvenance = isPaper ? {
        schema_version: 'paper_lane_provenance_v2_20260913',
        paper_lane: paperLane, // paper_primary | paper_analysis
        selection_source: paperLane === 'paper_analysis' ? 'operator_paper_analysis' : 'operator_paper_primary',
        paper_selection_source: paperLane === 'paper_analysis' ? 'operator_paper_analysis' : 'operator_paper_primary',
        evidence_source: paperLane === 'paper_analysis' ? 'operator_paper_analysis' : 'operator_paper_primary',
        paper_policy_version: paperPolicyVersion,
        brain_authorization_id: brainAuthId,
        brain_auth_source: paperLane === 'paper_analysis'
            ? (paperAnalysisAuth?.source || 'brain_paper_analysis_eligibility')
            : 'final_entry_authorization',
        ...
        paper_analysis_authorization: paperLane === 'paper_analysis' ? {
            allowed: paperAnalysisAuth?.allowed === true,
            reason: paperAnalysisAuth?.reason || null,
            source: paperAnalysisAuth?.source || null,
            authorization_id: brainAuthId,
        } : null,
        ...
        structural_contract: {
            ...
            leg_count: paperAuthorization.legCount ?? paperAuthorization.leg_count,
            lot_size: paperAuthorization.lotSize,
            number_of_lots: paperAuthorization.number_of_lots ?? entryNumberOfLots,
            quantity_units: paperAuthorization.quantity_units ?? entryQuantityUnits,
            index_key: paperAuthorization.indexKey || normalizePaperIndexKey(cand.index),
            identity_version: cand?.contract_identity?.schema_version
                || cand?.identity_version
                || 'contract_identity_v1_20260913',
            authorization_lot_size: entryLotSize,
        },
        entry_quotes: [
    ...
    const trade = {
        ...
        entry_snapshot: {
            ...
            ...(paperTestProvenance ? {
                paper_test: paperTestProvenance,
                paper_lane: paperTestProvenance.paper_lane,
        ...
        // Compact mirrors remain inside entry_snapshot only.
    };
...
async function triggerDayEvaluationForce() {
    ...
    const proceed = confirm(
        'Force evaluation on an incomplete session?\n\n' +
        'Use this only for advisory analysis. The result will be excluded from promotion gates and should not be treated as baseline-quality evidence.'
    );
    if (!proceed) return;
    try {
        ...
        alert(response.message || `Forced day evaluation started${response?.target_date ? ` for ${response.target_date}` : ''}. Refresh ML status in a few seconds.`);
    } catch (e) {
        alert('Forced day evaluation trigger failed: ' + e.message);
    }
}
...
function renderOI(snapshot = null) {
    ...
    const nfFP = l.futuresPremNf ?? nfc.futuresPremium;
...
function renderWatchlist(snapshot = null) {
    ...
    // Paper analysis alternatives — structurally recordable non-watchlist candidates.
    // Does not change Real authorization. Sole experimental lane remains Paper TEST.
    const watchlistIds = brainWatchlist.map(c => c?.id).filter(id => id != null);
    const nfAlt = paperAnalysisAlternatives('NF', watchlistIds, nfAtm);
    const bnfAlt = paperAnalysisAlternatives('BNF', watchlistIds, bnfAtm);
    if (nfAlt.length || bnfAlt.length) {
        html += `<details style="margin-top:10px"><summary style="cursor:pointer;font-size:12px;font-weight:600;color:var(--accent);padding:6px 0;user-select:none;">📋 PAPER ANALYSIS — non-primary alternatives (${nfAlt.length + bnfAlt.length}) ▸</summary>`;
        html += `<div class="section-note" style="font-size:11px;color:var(--text-muted);margin:0 0 8px 0;">Structurally valid candidates rejected by ranking/ML/final authority remain Paper-testable. Real/Sandbox buttons stay locked to the final brain selection.</div>`;
        nfAlt.forEach((c, i) => { html += renderCandidateCard(c, nfAtm, `A${i + 1}`); });
        bnfAlt.forEach((c, i) => { html += renderCandidateCard(c, bnfAtm, `B${i + 1}`); });
        html += '</details>';
    }
...
/** G9 Kelly card hidden until Python G9 sizing is wired safely — no lot-count readout. */
function experimentalKellyAdvisoryReadout(_cand) {
    return '';
}
...
function renderTradeCard(t, isPaper) {
    ...
    // No Paper-vs-teacher pooling path is active here. Show horizon and warning,
    // but do not claim a comparison filter was applied.
    const horizonNote = holdingMeta.intraday_carried_beyond_session ? `<div style="color:var(--warn);font-size:12px;margin-top:4px;">Data-quality warning: intraday mode but held beyond entry session (${holdingMeta.holding_horizon}). No Paper-vs-teacher pooling path is active.</div>` : '';
...
function renderML(snapshot = null) {
    ...
    // C3 corrective: labels-saved is a durable ledger fact only. Never infer it
    // from done date, phase, or aggregate counts. Absent/stale ledger is UNKNOWN.
    const labelsSaved = service.labelsSaved === true;
    const labelsSavedKnown = service.labelsSavedKnown === true || typeof service.labelsSaved === 'boolean';
    const labelsSavedText = labelsSaved ? 'YES' : (labelsSavedKnown ? 'NO' : 'UNKNOWN/PENDING');
    const evaluationPhaseRaw = String(service.evaluationPhase || '').toUpperCase();
    const incompleteIdentity = evaluationPhaseRaw === 'INCOMPLETE_IDENTITY' || evaluationPhaseRaw === 'FAILED_IDENTITY_COVERAGE';
    const missingIdentityCount = Number(service.missingIdentityCount || 0);
    const missingIdentityPreview = String(service.missingIdentityPreview || '');
    ...
    const evaluationAlarmFiredForTarget = evaluationTargetDate && String(service.evaluationAlarmFiredDate || '') === evaluationTargetDate;
    const evaluationAutoStartForTarget = evaluationTargetDate && String(service.evaluationAutoStartDate || '') === evaluationTargetDate;
    const evaluationAutoStartStatus = String(service.evaluationAutoStartStatus || '');
    const evaluationAutoStartError = String(service.evaluationAutoStartError || '');
    const evaluationAutoStartFailed = evaluationAutoStartForTarget && evaluationAutoStartStatus.toUpperCase() === 'FAILED';
    const evaluationAutoStartPending = evaluationAlarmFiredForTarget && !evaluationAutoStartForTarget && !evaluationDone && !evaluationRunning;
    ...
    const coverageIntegrity = String(service.coverageIntegrity || '');
    ...
    const evaluationFailureDetails = evaluationRetryable && !evaluationRunning
        ? `<br><span style="color:var(--warn)">Last attempt: ${escapeHtml(evaluationPhaseLabel || 'STOPPED')} · ${escapeHtml(evaluationProgressText)}<br>${escapeHtml(service.evaluationLastError || evaluationMessage || 'Open Logs for failure details.')}</span>`
        : '';
    ...
    const evaluationButtonText = incompleteSession && evaluationForceAllowed
        ? '⚠ Force Eval'
        : (evaluationDone && !evaluationRetryable)
            ...
            : (!evaluationReady
                ? (evaluationBlockedReason === 'WAIT_FOR_POST_CLOSE_HANDOFF'
                    ...
                    : evaluationBlockedReason === 'SESSION_PARTIAL'
                        ...
                        : evaluationBlockedReason === 'INCOMPLETE_SESSION'
                            ? '⛔ Incomplete Session'
                        : '⛔ Not Ready')
                : `📋 Evaluate ${evaluationTargetIsToday ? 'Today' : (evaluationTargetLabel || 'Session')}`)));
    const evaluationButtonDisabled = evaluationRunning || (incompleteSession ? !evaluationForceAllowed : ((evaluationDone && !evaluationRetryable) || !evaluationReady));
    const evaluationButtonAction = incompleteSession && evaluationForceAllowed ? 'triggerDayEvaluationForce()' : 'triggerDayEvaluation()';
    ...
    const teacherLaneStats = hasNativeTeacherSummary ? {
        NF_intraday: { rows: Number(summaryTeacherLanes.NF_intraday?.rows || 0), successes: Number(summaryTeacherLanes.NF_intraday?.successes || 0), successRatePct: Number(summaryTeacherLanes.NF_intraday?.successRatePct || 0), expectancyR: Number(summaryTeacherLanes.NF_intraday?.expectancyR || 0), avgCapturedPct: Number(summaryTeacherLanes.NF_intraday?.avgCapturedPct || 0), breakEvenWinRatePct: Number(summaryTeacherLanes.NF_intraday?.breakEvenWinRatePct || 0), worthTrading: summaryTeacherLanes.NF_intraday?.worthTrading === true, sampleUncertain: summaryTeacherLanes.NF_intraday?.sampleUncertain === true || summaryTeacherLanes.NF_intraday?.sample_uncertain === true, distinctSessionCount: Number(summaryTeacherLanes.NF_intraday?.distinctSessionCount ?? summaryTeacherLanes.NF_intraday?.distinct_session_count ?? 0) },
        NF_swing: { rows: Number(summaryTeacherLanes.NF_swing?.rows || 0), successes: Number(summaryTeacherLanes.NF_swing?.successes || 0), successRatePct: Number(summaryTeacherLanes.NF_swing?.successRatePct || 0), expectancyR: Number(summaryTeacherLanes.NF_swing?.expectancyR || 0), avgCapturedPct: Number(summaryTeacherLanes.NF_swing?.avgCapturedPct || 0), breakEvenWinRatePct: Number(summaryTeacherLanes.NF_swing?.breakEvenWinRatePct || 0), worthTrading: summaryTeacherLanes.NF_swing?.worthTrading === true, sampleUncertain: summaryTeacherLanes.NF_swing?.sampleUncertain === true || summaryTeacherLanes.NF_swing?.sample_uncertain === true, distinctSessionCount: Number(summaryTeacherLanes.NF_swing?.distinctSessionCount ?? summaryTeacherLanes.NF_swing?.distinct_session_count ?? 0) },
        BNF_intraday: { rows: Number(summaryTeacherLanes.BNF_intraday?.rows || 0), successes: Number(summaryTeacherLanes.BNF_intraday?.successes || 0), successRatePct: Number(summaryTeacherLanes.BNF_intraday?.successRatePct || 0), expectancyR: Number(summaryTeacherLanes.BNF_intraday?.expectancyR || 0), avgCapturedPct: Number(summaryTeacherLanes.BNF_intraday?.avgCapturedPct || 0), breakEvenWinRatePct: Number(summaryTeacherLanes.BNF_intraday?.breakEvenWinRatePct || 0), worthTrading: summaryTeacherLanes.BNF_intraday?.worthTrading === true, sampleUncertain: summaryTeacherLanes.BNF_intraday?.sampleUncertain === true || summaryTeacherLanes.BNF_intraday?.sample_uncertain === true, distinctSessionCount: Number(summaryTeacherLanes.BNF_intraday?.distinctSessionCount ?? summaryTeacherLanes.BNF_intraday?.distinct_session_count ?? 0) },
        BNF_swing: { rows: Number(summaryTeacherLanes.BNF_swing?.rows || 0), successes: Number(summaryTeacherLanes.BNF_swing?.successes || 0), successRatePct: Number(summaryTeacherLanes.BNF_swing?.successRatePct || 0), expectancyR: Number(summaryTeacherLanes.BNF_swing?.expectancyR || 0), avgCapturedPct: Number(summaryTeacherLanes.BNF_swing?.avgCapturedPct || 0), breakEvenWinRatePct: Number(summaryTeacherLanes.BNF_swing?.breakEvenWinRatePct || 0), worthTrading: summaryTeacherLanes.BNF_swing?.worthTrading === true, sampleUncertain: summaryTeacherLanes.BNF_swing?.sampleUncertain === true || summaryTeacherLanes.BNF_swing?.sample_uncertain === true, distinctSessionCount: Number(summaryTeacherLanes.BNF_swing?.distinctSessionCount ?? summaryTeacherLanes.BNF_swing?.distinct_session_count ?? 0) }
    } : {
    ...
    // Batch D corrective: chosen reporting comes only from native teacher_summary,
    // whose row/session/rate metrics share one primary teacher_v1 gradeable population.
    const teacherDistinctSessions = hasNativeTeacherSummary ? Number(
        summaryTeacher?.distinctSessionCount ?? summaryTeacher?.distinct_session_count ?? 0
    ) : null;
    const teacherNetProfitableRate = hasNativeTeacherSummary ? (
        summaryTeacher?.netProfitableRatePct ?? summaryTeacher?.net_profitable_rate ?? null
    ) : null;
    const teacherSampleUncertain = hasNativeTeacherSummary
        ? (summaryTeacher?.sampleUncertain === true || summaryTeacher?.sample_uncertain === true)
        : true;
    const teacherProfitabilityVerdict = teacherSampleUncertain
        ? null
        : (summaryTeacher?.profitabilityVerdict ?? summaryTeacher?.profitability_verdict ?? null);
    ...
    const evaluationIncompleteMessage = incompleteSession
        ? `Session integrity is broken${coverageIntegrityIssue ? ` (${coverageIntegrityIssue})` : ''}. Normal teacher evaluation is blocked for this session.`
        : '';
    ...
    const researchPendingMessage = postCloseArtifactsPending
        ...
        : (incompleteSession
            ? 'No teacher research report was generated because this session failed integrity checks before normal post-close evaluation.'
    ...
    const teacherReportPendingNotFailed = postCloseArtifactsPending && teacherResearchStatus === 'FAILED' && teacherResearchError === 'LOCAL_REPORT_NOT_AVAILABLE';
    ...
    const researchIntegrity = safeParseNB(teacherResearchReport?.integrity_summary, teacherResearchReport?.integrity_summary || {});
    ...
    const researchStrategyLine = Object.entries(researchByStrategy || {})
        .filter(([, row]) => Number(row?.rows || 0) > 0)
        .sort((a, b) => Number(b[1]?.rows || 0) - Number(a[1]?.rows || 0))
        .slice(0, 3)
        .map(([key, row]) => `${key} ${Number(row?.rows || 0)} @ ${researchFmt(row?.avg_r, 2, 'R')}`)
        .join(' · ') || '--';
    const researchIntegrityCounts = safeParseNB(researchIntegrity?.counts, researchIntegrity?.counts || {});
    const researchGradeableTeacherRows = Number(researchIntegrity?.gradeable_teacher_rows || 0);
    const researchGradeablePrimaryRows = Number(researchIntegrity?.gradeable_primary_rows || 0);
    const researchFailIntegrityRows = Number(researchIntegrityCounts?.FAIL || 0);
    const researchNoGradeableTeacherOutcomes = researchOk && Number(teacherResearchReport?.outcome_count || 0) > 0 && researchGradeableTeacherRows === 0;
    ...
    const stage2aAuditPendingMessage = postCloseArtifactsPending
        ...
        : (incompleteSession
            ? 'Stage 2A shadow audit is not applicable because this session failed integrity checks before normal post-close evaluation.'
        : (researchArtifactMissing
    ...
    const classAGateStatus = String(
        postCloseArtifactsPending ? 'PENDING' : (classAGateHasData ? classAGate.status : (incompleteSession ? 'N/A' : (researchArtifactMissing ? 'UNAVAILABLE' : (evaluationEmptySession ? 'N/A' : 'FAIL'))))
    ).toUpperCase();
        ...
        <section class="section">
            ...
            <div class="brain-card" style="border-left-color:${researchOk ? 'var(--accent)' : 'var(--warn)'}">
                ...
                <div class="brain-detail">
                    ${researchOk ? `
                        ...
                        Chosen teacher: <b>${Number(researchPrimary.rows || 0)}</b> rows · <b>${researchFmt(researchPrimary.avg_r, 2, 'R')}</b> avg · Teacher target hit <b>${researchFmt(researchPrimary.success_rate_pct, 1, '%')}</b><br>
                        ...
                        Strategy outcomes: <b>${researchStrategyLine}</b>${researchNoGradeableTeacherOutcomes ? `<br><span style="color:var(--warn)">No gradeable teacher outcomes were available in this session.${researchFailIntegrityRows > 0 ? ` ${researchFailIntegrityRows} rows failed integrity.` : ''}</span>` : ''}${researchGradeableTeacherRows > 0 && researchGradeablePrimaryRows === 0 ? `<br><span style="color:var(--warn)">Teacher outcomes exist, but none belong to chosen primary rows yet.</span>` : ''}
                    ` : `
            ...
            <div class="brain-card" style="border-left-color:${classAGateColor}">
                ...
                <div class="brain-detail">
                    ...
                    Outcome rows: <b>${Number(classAGate?.outcome_count || 0)}</b> · Chosen rows: <b>${Number(classAGate?.primary_outcome_count || 0)}</b>${postCloseArtifactsPending ? `<br><span style="color:var(--accent)">This gate is evaluated only after post-close teacher research is generated for today's session.</span>` : ''}${incompleteSession ? `<br><span style="color:var(--accent)">No class-A comparison applies because this session failed integrity checks before normal evaluation.</span>` : ''}${evaluationEmptySession || classAGate?.no_class_a_session ? `<br><span style="color:var(--accent)">No class-A comparison applies for this session because the brain produced no evaluable candidate legs.</span>` : ''}${classAGateHasData && classAGateStatus !== 'N/A' ? `<br><span style="color:${classAGateReady ? 'var(--green)' : 'var(--warn)'}">${classAGateReady ? 'Baseline is complete enough for tomorrow comparison.' : 'Baseline is not complete for tomorrow comparison yet.'}</span>` : ''}${classAGateBlocked.length > 0 ? `<br><span style="color:var(--warn)">Blocked: ${classAGateBlocked.join(' · ')}</span>` : ''}
                </div>
            </div>
            ...
            <div class="brain-card" style="border-left-color:var(--warn)">
                ...
                <div class="brain-detail">
                    ${incompleteSession ? `
                        ...
                        Candidate diagnostics are advisory only because this session failed integrity checks before normal evaluation.
            ...
            <div class="brain-card" style="border-left-color:var(--accent)">
                ...
                <div class="brain-detail">
                    Chosen rows: <b>${teacherLaneTotal}</b> · Distinct sessions: <b>${teacherDistinctSessions != null ? teacherDistinctSessions : '--'}</b> · Teacher target hit: <b>${teacherLaneTotal > 0 ? `${teacherSuccessRatePct.toFixed(1)}%` : '--'}</b> · Net profitable: <b>${teacherNetProfitableRate != null && teacherLaneTotal > 0 ? `${Number(teacherNetProfitableRate).toFixed(1)}%` : '--'}</b> · Sample: <b>${teacherSampleUncertain ? 'UNCERTAIN' : 'MULTI-SESSION'}</b>${teacherProfitabilityVerdict ? ` · Profitability verdict: <b>${escapeHtml(String(teacherProfitabilityVerdict))}</b>` : ''}<br>
                    ...
                    Avg captured: <b>${teacherLaneTotal > 0 ? `${teacherAvgCapturedPct.toFixed(1)}%` : '--'}</b> · Bucket gate: <b>${teacherBucketCount > 0 ? `${teacherTradeableBucketCount}/${teacherBucketCount}` : '--'}</b>${teacherBucketCount > 0 && !teacherSampleUncertain ? ` · Verdict: <b style="color:${teacherWorthTrading ? 'var(--green)' : 'var(--warn)'}">${teacherWorthTrading ? 'POSITIVE EXPECTANCY' : 'NOT WORTH RISK YET'}</b>` : (teacherBucketCount > 0 ? ' · Verdict withheld: <b>INSUFFICIENT SESSIONS</b>' : '')}<br>
            ...
            <div class="brain-card" style="border-left-color:var(--warn)">
                ...
                <div class="brain-detail">
                    Legacy chosen win rate: <b>${comparisonLegacyRows > 0 ? `${comparisonLegacyWinRatePct.toFixed(1)}%` : '--'}</b> · Teacher target hit: <b>${comparisonTeacherRows > 0 ? `${comparisonTeacherSuccessRatePct.toFixed(1)}%` : '--'}</b><br>
            ...
            <div class="brain-card" style="border-left-color:var(--accent)">
                ...
                <div class="brain-detail">
                    Teacher metrics are expectancy-first. Teacher target hit means the managed exit actually captured the target; net profitability is reported separately.
                    ...
                    <table style="width:100%;border-collapse:collapse;font-size:12px">
                        ...
                        <tbody>
                            ${[
                                ...
                                const worth = lane.rows > 0 ? (lane.sampleUncertain ? 'UNCERTAIN' : (lane.worthTrading ? 'YES' : 'NO')) : '--';
                                return `
                                    <tr>
                                        <td style="padding:6px 4px;border-top:1px solid var(--border)"><b>${label}</b></td>
                                        ...
                                        <td style="padding:6px 4px;border-top:1px solid var(--border);text-align:right;color:${lane.worthTrading && !lane.sampleUncertain ? 'var(--green)' : 'var(--warn)'}">${worth}</td>
                                    </tr>
                                `;
                            }).join('')}
                        </tbody>
