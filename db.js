/* ═══════════════════════════════════════════════════════════════
   ...
   Tables: premium_history, trades_v2, chain_snapshots, app_config
...
const DB = (() => {
    ...
    // ═══ APP CONFIG — device-independent state persistence ═══
    // Key-value store: morning_inputs, morning_bias, global_direction, settings

    async function setConfig(key, value) {
        if (!init()) { localStorage.setItem('mr2_cfg_' + key, JSON.stringify(value)); return null; }
        try {
            // Always save localStorage as offline cache
            localStorage.setItem('mr2_cfg_' + key, JSON.stringify(value));
            const { data, error } = await sb
                .from('app_config')
                .upsert({ key, value, updated_at: new Date().toISOString() }, { onConflict: 'key' });
            if (error) console.warn('[DB] setConfig error:', key, error.message);
            return data;
        } catch (e) {
            console.warn('[DB] setConfig exception:', key, e.message);
            return null;
        }
    }
    ...
    async function getConfig(key) {
        if (!init()) {
            try { return JSON.parse(localStorage.getItem('mr2_cfg_' + key)); } catch { return null; }
        }
        try {
            const { data, error } = await sb
                .from('app_config')
            ...
            localStorage.setItem('mr2_cfg_' + key, JSON.stringify(data.value));
            return data.value;
        } catch (e) {
            try { return JSON.parse(localStorage.getItem('mr2_cfg_' + key)); } catch { return null; }
        }
    ...
    async function getAllConfig() {
        if (!init()) {
            // Offline: gather from localStorage
            const result = {};
            for (const k of ['morning_inputs', 'morning_bias', 'global_direction', 'settings']) {
                try { result[k] = JSON.parse(localStorage.getItem('mr2_cfg_' + k)); } catch { result[k] = null; }
            }
            return result;
        }
        try {
            ...
            for (const row of data) {
                ...
                // Sync to localStorage cache
                localStorage.setItem('mr2_cfg_' + row.key, JSON.stringify(row.value));
            }
            return result;
        } catch (e) {
            const result = {};
